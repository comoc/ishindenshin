...
筋萎縮性側索硬化症 (ALS)・多系統萎縮症 (MSA) などのために、身体の動作が限定的な方が **1つのスイッチ入力（走査入力）** だけで日本語文章を作成できるWebアプリです。On / Off の信号のみで、50音表からの文字選択・濁点／半濁点・小書き・走査方向の反転までを行えます。
...
- **スイッチ**: `Space` / `Enter`、または DualSense（○ ✕ △ □）／ Xbox（A B X Y）コントローラーのフェイスボタン
- **走査速度（速く／遅く）**: `→` / `←`
- **一時停止／再開**: `P`
- **メインメニューへ戻る**: `Esc`
...
- 入力デバイス（任意。以下のいずれかが利用できれば操作可能）
  - キーボード相当の外部スイッチ（`Space` / `Enter` を発火するもの）
  - DualSense / Xbox 互換のゲームパッド（○ ✕ △ □ / A B X Y のいずれかのフェイスボタン）
  - 介護者によるマウス／タップ操作
